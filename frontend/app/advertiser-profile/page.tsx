...
const AdvertiserProfile: NextPage = () => {
  ...
  async function checkAdvertiserById(advertiserId: string) {
    try {
      const response = await fetch(`https://prisma-tech-mac-backend.vercel.app/announcers/${advertiserId}`, {
        method: "GET",
        headers: {
  ...
  async function updateAdvertiser(id: string, updateData: any) {
    try {
      ...
      const updateResponse = await fetch(`https://prisma-tech-mac-backend.vercel.app/announcers/${id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
