...
const CreatorProfile: NextPage = () => {
  ...
  async function checkCreatorById(creatorId: string) {
    try {
      const response = await fetch(`https://prisma-tech-mac-backend.vercel.app/creators/${creatorId}`, {
        method: "GET",
        headers: {
  ...
  async function updateCreator(id: string, updateData: any) {
    try {
      const updateResponse = await fetch(`https://prisma-tech-mac-backend.vercel.app/creators/${id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
